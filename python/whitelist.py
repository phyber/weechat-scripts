...
def whitelist_check(message):
    """Return a boolean indicating if the sender is whitelisted or not."""
    nick = message.nick()
    host = message.host()
...
def whitelist_cmd(userdata, buf, args):
    ...
    if listtype in VALID_OPTION_TYPES:
        ...
        if arg is not None:
            ...
            weechat.prnt(
                "",
                "Error. Must supply an argument to '{type}'.".format(
                    type=listtype))
    else:
        weechat.prnt(
            "",
            "Error. Valid whitelist types are: {types}.".format(
